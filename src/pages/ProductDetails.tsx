...
import { ArrowLeft, ShoppingCart, Truck, PhoneCall } from 'lucide-react';
import { useStore } from '../store';
import { BGN_RATE } from '../constants';

export const ProductDetails = () => {
  const { id } = useParams();
  const { products, addToCart, openAssistant } = useStore();
        ...
        <div className="grid md:grid-cols-2 gap-0">
          ...
          <div className="p-8 md:p-16 flex flex-col justify-center">
            ...
            {product.price > 0 ? (
                ...
                <div className="text-4xl font-heading font-bold text-orange-600 mb-8">
                  Цена по договаряне
                </div>
            )}
            ...
            <div className="border-t border-slate-100 pt-10 mt-auto">
              <div className="flex flex-col sm:flex-row gap-5">
                ...
                <button onClick={handleAction} className={`flex-1 text-white px-8 py-4 rounded-full font-bold uppercase tracking-wider transition shadow-lg flex items-center justify-center gap-3 transform active:scale-95 touch-manipulation ${product.price > 0 ? 'bg-gradient-to-r from-neutral-900 to-neutral-800 hover:from-orange-600 hover:to-red-600' : 'bg-orange-600 hover:bg-orange-700'}`}>
